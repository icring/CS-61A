...
# Email: stheakanath@berkeley.edu
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            #Working forward figure out how to code this without repeat code
            while curr_place != None:
                if curr_place.ant != None and curr_place.ant not in self.ants and curr_place.ant is not self:
                    self.ants.append(curr_place.ant)
                    curr_place.ant.damage *= 2
                    if curr_place.ant.name == 'Bodyguard' and curr_place.ant.ant is not None and curr_place.ant.ant is not self:
...
def apply_effect(effect, bee, duration):
    ...
    def apply_status_effect(colony):
        nonlocal duration
        if duration > 0:
        ...
        duration -= 1
    bee.action = apply_status_effect
