...
def shuffle(cards):
    ...
    "*** YOUR CODE HERE ***"
    arr1, arr2 = cards[len(cards)//2:], cards[:len(cards)//2]
    new = []
...
def trade(first, second):
    ...
    def return_sum(arr):
        ...
        for x in range(len(arr)):
            ...
            sum.append(total)
        return sum
    first_sum, second_sum = return_sum(first), return_sum(second)
    index, index2, m, n =  [i for i, item in enumerate(first_sum) if item in set(second_sum)], [i for i, item in enumerate(second_sum) if item in set(first_sum)], 1, 1
    if len(index) > 0:
        m, n = index[0]+1, index2[0]+1
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attempts, times = [], 0
    def withdraw(amount, attempted_password):
        nonlocal balance, attempts, times
        if times == 3:
            return "Your account is locked. Attempts: " + str(attempts)
        if attempted_password != password:
            attempts, times = attempts + [attempted_password], times + 1
            if times > 3:
                return "Your account is locked. Attempts: " + str(attempts)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(err) == str:
        return err
    def attempt(amount, attempted_password):
