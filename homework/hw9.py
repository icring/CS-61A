...
def brack_read(tokens):
    ...
    if n != None:
        ...
        "*** YOUR CODE HERE ***"
        target_token = left_to_right.get(token)
        item = brackets.get((token, target_token))
        return Pair(item, read_tail(tokens, target_token))

def exclude(end_token):
    items = ['}', ']', ')', '>']
    items.remove(end_token)
    return items

def read_tail(src, target_token = None):
    if len(src) == 0:
        raise SyntaxError("unexpected end of line")
    if src[0] in exclude(target_token):
        raise SyntaxError('unexpected ' + src[0])
    if src[0] == target_token:
        src.pop(0)
        return nil
    first = brack_read(src)
    rest = read_tail(src, target_token)
...
class Mobile:
    ...
    def is_balanced(self):
        ...
        "*** YOUR CODE HERE ***"
        base_line = self.left.torque == self.right.torque
        left_contents, right_contents = self.left.contents, self.right.contents
        return base_line and left_contents.is_balanced() and right_contents.is_balanced()
...
def interpret_mobile(s):
    ...
    for t in ('Branch', 'Weight', 'Mobile'):
        "*** YOUR CODE HERE ***"
        test_mobile = interpret_mobile(s[:next_T] + t + s[next_T + 1:])
...
class Stream:
    ...
    def __iter__(self):
        ...
        "*** YOUR CODE HERE ***"
        def iteartor(scm):
            while scm != Stream.empty:
                yield scm.first
                scm = scm.rest
        return iteartor(self)
